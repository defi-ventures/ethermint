...
import (
	...
	srvflags "github.com/defi-ventures/ethermint/server/flags"
	ethermint "github.com/defi-ventures/ethermint/types"
	evmtypes "github.com/defi-ventures/ethermint/x/evm/types"

	"github.com/defi-ventures/ethermint/testutil/network"
)
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", ethermint.AttoBcx), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(ethermint.AttoBcx, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(ethermint.AttoBcx, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(ethermint.AttoBcx)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, ethermint.AttoBcx, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
