...
"$PWD"/build/blockxd keys add $KEY --keyring-backend test --algo "eth_secp256k1"
...
cat $HOME/.ethermint/config/genesis.json | jq '.app_state["crisis"]["constant_fee"]["denom"]="abcx"' > $HOME/.ethermint/config/tmp_genesis.json && mv $HOME/.ethermint/config/tmp_genesis.json $HOME/.ethermint/config/genesis.json
cat $HOME/.ethermint/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="abcx"' > $HOME/.ethermint/config/tmp_genesis.json && mv $HOME/.ethermint/config/tmp_genesis.json $HOME/.ethermint/config/genesis.json
cat $HOME/.ethermint/config/genesis.json | jq '.app_state["mint"]["params"]["mint_denom"]="abcx"' > $HOME/.ethermint/config/tmp_genesis.json && mv $HOME/.ethermint/config/tmp_genesis.json $HOME/.ethermint/config/genesis.json

# Allocate genesis accounts (cosmos formatted addresses)
"$PWD"/build/blockxd add-genesis-account "$("$PWD"/build/blockxd keys show "$KEY" -a --keyring-backend test)" 100000000000000000000abcx,10000000000000000000stake --keyring-backend test

# Sign genesis transaction
"$PWD"/build/blockxd gentx $KEY 10000000000000000000stake --amount=100000000000000000000abcx --keyring-backend test --chain-id $CHAINID
...
"$PWD"/build/blockxd start --pruning=nothing --rpc.unsafe --json-rpc.address="0.0.0.0:8545" --keyring-backend test > blockxd.log 2>&1 &

# Give blockxd node enough time to launch
sleep 5
...
#PRIVKEY="$("$PWD"/build/blockxd keys export $KEY)"
...
# After tests
# kill test blockxd
echo "going to shutdown blockxd in 3 seconds..."
sleep 3
pkill -f "ethermint*"
