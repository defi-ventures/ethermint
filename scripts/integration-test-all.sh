...
init_func() {
    "$PWD"/build/blockxd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/blockxd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/blockxd add-genesis-account \
    "$("$PWD"/build/blockxd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000abcx,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/blockxd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
