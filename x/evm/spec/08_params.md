...
For example, on Ethereum, the `evm_denom` would be `ETH`. In the case of Ethermint, the default denomination is the **[atto bcx](notion://www.notion.so/docs/basics/bcx.md)** (used on the Evmos testnets). In terms of precision, the `BCX` and `ETH` share the same value, *i.e* `1 BCX = 10^18 atto bcx` and `1 ETH = 10^18 wei`.

::: tip
Note: SDK applications that want to import the EVM module as a dependency will need to set their own `evm_denom` (i.e not `"abcx"`).
:::
